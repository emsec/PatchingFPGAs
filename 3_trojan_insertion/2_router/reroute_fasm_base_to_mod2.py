...
v2: While rerouting, if there are already known solutions for the sink INT
tiles (starting from the end of the first PIP that is on both ends in this INT)
use these instead of best effort routing. This solves an issue where almost no
routes can be found if multiple signals do split heavily in a single INT.
Reason is that otherwise the first net gets routed optimal which always works,
then the second net targeted on the same "crowded" INT will very likely fail as
there might be not enough free different resources to be able to route it... 
"""
import re
import sys
            ...
            if re.match(r"^CLB.*_IMUX[0-9]+$", nextWire.getWireName()):
                ...
                for pip in fasmPIPs:
                    if pip.getEndWire() == nextWire or (pip.getStartWire() == nextWire and pip.isBidirectional()):
                        ...
                        q.add(nextWire)
                        pips.add(pip)
                        if len(wire.getBackwardPIPs()) == 1:
                            # we originate from a "single" PIP, so add this one to the set of pips
                            pips.update(wire.getBackwardPIPs())
                        break
                else:
                    if len(RouteNode(nextWire.getTile(), nextWire.getWireIndex()).getBackwardConnections()) == 1 and not re.match(r"^CLB.*_(?:[LM]|LL)_[A-D](?:MUX)?$", nextWire.getWireName()):
                        ...
                        q.add(nextWire)
    # go through each sink, return the destination int nodes, so each last INT tile doesn't need to get rerouted
    preset_pips = set()
    routeNodes = set()
    for sink in sinks:
        # TODO: check if the previous source of the net is in the same INT tile. If so, we need to reroute anyway! (Luckily this didn't occur so far)
        tile = sink.getTile()
        inttile = tile.getTileNeighbor(1 if "_L_" in tile.getName() else -1, 0)
        tile_pips = {pip for pip in pips if pip.getStartWire().getTile() in [tile, inttile] and pip.getEndWire().getTile() in [tile, inttile]}
        if not tile_pips:
            routeNodes.add(RouteNode(tile, tile.getWireFromSitePin(sink)))
            print("Error: This version requires routed signals, problematic sink / tile is:", sink, tile, "Solution: use the sink as target routenode")
        else:
            #routeNodes |= {pip.getStartRouteNode() for pip in tile_pips}
            for pip in tile_pips:
                startWire = pip.getStartWire()
                previousWire = startWire.getStartWire()
                previousTile = previousWire.getTile()
                if previousTile == inttile:
                    continue
                routeNodes.add(RouteNode(previousTile, previousTile.getWireIndex(previousWire.getWireName())))
            global sink_
            sink_ = sink
            # go back from the CLB sink to the INT node to not miss that part of the routing
            pips_analysis = tile.getBackwardPIPs(tile.getWireFromSitePin(sink))
            start_wire = pips_analysis[0].getStartWire()
            previous_wire = tile.getBackwardConnections(start_wire.getWireIndex())[0]
            previous_backward_pips = previous_wire.getBackwardPIPs()
            if len(previous_backward_pips) == 1:
                pips_analysis.append(previous_backward_pips[0])
            print("PIPs to be analyzed:", pips_analysis)
            tile_pips |= set(pips_analysis)
        print("sink:", sink, tile, inttile, tile_pips, routeNodes)
        preset_pips |= tile_pips

    # go through all routenodes (A)
    # go over all preset pips, check if on our way we find another routenode (B)
    # in this case nothing needs to be done to route to B, remove B as routeNode.
    routeNodesToRemove = set()
    for routeNode in routeNodes:
        if set(routeNode.getBackwardPIPs()) & preset_pips:
            routeNodesToRemove.add(routeNode)
    print("RNs to remove: "+str(routeNodesToRemove))
    print("new routeNodes:",(routeNodes - routeNodesToRemove))
    routeNodes -= routeNodesToRemove

    #pips -= tile_pips # do not remove from the pips that count to the net, as pips can still be used to route to other targets
    global pips_
    pips_ = preset_pips
    return pips, sinks, preset_pips, routeNodes
...
def elaborateRerouteNets(design, fasmPIPDict, usedNodes, intraSiteRoutes):
    ...
    for cell in design.getCells():
        if not re.match(r"^o_[0-9]+(_lutmod_[0-9A-F]+)?(/LUT[56])?$", cell.getName()):
            ...
            continue
        verbose = True #(cell.getName() == "o_14" or cell.getName() == "o_19")
        outputPorts = []
        ...
        for outputPort in outputPorts:
            ...
            pipsToRemove, sinks, preset_pips, routeNodes = fasmFindPIPsAndSinksFromSinkPinInst(initialSitePin, fasmPIPDict, verbose)
            routeTargetsPips[net] = (preset_pips, routeNodes)
    ...
    for cellToCreateArgs, cellToCreateNet, cellToCreatePinName in cellsToCreate:
        ...
        if isLUT:
            ...
            if lutEquation == "O=0":
                lutEquation = " " + lutInput # "O =" can be omitted here
            elif lutInput not in lutEquation:
                lutEquation += " + " + lutInput
            print("set " + str(cell) + "LUT INIT to " + lutEquation)
            LUTTools.configureLUT(cell, lutEquation)
        else:
            lutInput = "D"
    ...
    return fasmOutput, routeTargetsPips

def routeDesign(design, usedNodes=None, fasm=None, routeTargetsPips={}, netPriorityList=[]):
    fasmOutput = ""
    ...
    for n in primaryNets + [n for n in design.getNets() if n not in primaryNets]:
        if n.isClockNet() or "_CLK" in n.name.upper() or "CLK_" in n.name.upper(): continue # FIXME better detection of Clock nets than "_CLK" in the name
        ...
        if n.isStaticNet():
            ...
            if n in routeTargetsPips:
                routeNet(n, usedNodes, fasm, design, routeTargetsPips[n][0], routeTargetsPips[n][1])
            else:
                routeNet(n, usedNodes, fasm, design, routeNodes=set(pin.getRouteNode() for pin in n.getSinkPins()))
...
class RoutingFailedError(Exception):
    def __init__(self, problematicNet, problematicRouteNode):
        self.net = problematicNet
        self.routeNode = problematicRouteNode
        self.message = "Failed to route: {} ({})".format(self.net.getName(), repr(self.routeNode))
        super(Exception, self).__init__(self.message)

def routeNet(net, usedNodes, fasm, design=None, preset_pips=set(), routeNodes=set()):
    ...
    srcNode = net.getSource().getRouteNode()
    ...
    # For each preset PIP start RouteNode, run findRoute()
    for routeNode in routeNodes:
        ...
        if routeNode in q:
            # nothing to do
            continue
        print("find Route to {}".format(routeNode))
        path, level = findRoute(q,routeNode, usedNodesCopy, routeThroughs=False, verbose=False, netName=net.name) #(net.name == "instr_rdata_o[15]")) # verbose=(sink.getSite().getName() == "SLICE_X149Y96" and sink.getName() == "A5"))
        if path == None:
            print("no path for routeNode",routeNode)
            raise RoutingFailedError(net, routeNode)
        elif path: # if path is empty, nothing seems to be required!
            print("resulting path for routeNode",routeNode,"is",path)
            # save longest route in the design
            global maxLevel
            if maxLevel < level:
                global maxLevelNetRouteNode
                maxLevel = level
                maxLevelNetRouteNode = (net, routeNode)
            ...
            if initiallyUsedPIPs.size() > 0:
                ...
                while pip.getStartRouteNode() != srcNode:
                    ...
                    if len(back):
                        ...
                        break
            # try to follow the path using our preset_pips, so we can finalize that route and never get routes we do use twice
            global path_, preset_pips_
            path_ = path
            preset_pips_ = preset_pips
            q2 = [path[-1]]
            while len(q2):
                elem = q2[0]
                q2 = q2[1:]
                forward = elem.getEndRouteNode().getConnections()
                for pip in preset_pips:
                    if any(wire == pip.startWire for wire in forward):
                        print("APPEND",pip)
                        path = list(path) + [pip] # BUG somehow we need to make a good new list here otherwise saving the design will fail
                        q2.append(pip)
    ...
    # look for and remove antennas while going through all the PIPs now
    global srcNode_, allUsedPIPsOutput_
    srcNode_ = srcNode
    allUsedPIPsOutput_ = allUsedPIPsOutput
    #if net.getName() == "instr_rdata_o[1]": quit()
    antennaPIPs = set(pip for pip in allUsedPIPsOutput if not any(p in allUsedPIPsOutput for p in pip.endNode.allDownhillPIPs) and not pip.tile in set(n.tile for n in pip.endNode.allDownhillNodes) and not pip.tile.name.startswith("CLB"))
    print("Antenna PIPs: {}".format(antennaPIPs))
    allUsedPIPsOutput -= antennaPIPs
...
def findRoute(q, snk, usedNodes, verbose=False, routeThroughs=True, netName=""):
    # We'll keep track of where we have visited and a watchdog timer
    #if verbose:
    #    print("UsedNodes:", usedNodes)
    visited = HashSet()
    watchdog = 5000000 # last 2 zero

    #visited.update(q) # all of the nodes already in the queue count as visited, so these will never get routed to again (does somehow not work as expected... :( )
    ...
    while(not q.isEmpty()):
        ...
        for wire in curr.getConnections():
            ...
            if (nextNode.getName() == "INT_R_X71Y61/BYP_ALT0" and snk.getName() == "CLBLL_R_X71Y61/CLBLL_LL_AX") or \
                ...
                (nextNode.getName() == "INT_R_X71Y58/BYP_ALT1" and netName != "instr_addr_i[4]"):
                ...
                continue
            # FIXME: another unknown reason leads to this being conflicting. These are undetected antennas (or maybe only existing nets?)
            # There are obviously some antennas left, but actually we tried to detect those, how come?
            if (nextNode.getName() == "INT_R_X55Y78/ER1BEG1" and netName == "instr_rdata_o[10]"): #or \
                #(nextNode.getName() == "INT_R_X59Y77/NE2BEG2" and netName == "instr_rdata_o[7]"):
                print("HERE, WHY IS THIS USED?")
                continue
...
def fasm_used_routing_resources(fasm, device):
    ...
    for line in fasm:
        if line.startswith('INT_') or line.startswith('CLB'):
            ...
            #if fasmtile == "INT_R_X85Y85" and fasmsink == "GFAN1":
            #    print("FASM: {}".format(line))
            #    global fasmnode_
            #    fasmnode_ = RouteNode(tile, tile.getWireIndex(fasmsink))
...
if __name__ == '__main__':
    ...
    fasmOutput, routeTargetsPips = elaborateRerouteNets(design, fasmPIPDict, prohibitedRouteNodes, intraSiteRoutes)
    fasmFile = open(sys.argv[4], 'w')
    fasmFile.write(fasmOutput)

    print("Route Design...")
    maxLevel = -1
    maxLevelNetRouteNode = None, None
    #Router(design).routeDesign() # - Use the RapidWright Router instead
    if len(sys.argv) == 5:
        fasmOutput = routeDesign(design, prohibitedRouteNodes, fasmPIPDict, routeTargetsPips)
    else:
        ...
        while True:
            ...
            try:
                ...
                if netPriorityList:
                    ...
                    designPriorities = []
                fasmOutput = routeDesign(design, usedNodes, fasmPIPDict, routeTargetsPips, designPriorities)
                break
    ...
    print("Longest Route is of Net " + maxLevelNetRouteNode[0].name + " to RouteNode " + repr(maxLevelNetRouteNode[1]) + " with level " + str(maxLevel))
